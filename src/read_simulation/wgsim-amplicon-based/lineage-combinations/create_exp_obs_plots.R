...
colnames(results_comb_names) <- c("read_cnt_1","read_cnt2","obs_lin1","obs_lin2","obs_abun1",
...
colnames(results_comb_names) <- c("isolate1","exp_lin1","read_cnt_1","read_cnt_2",
                                  "obs_lin1","obs_lin2","obs_abun1",
                                 "obs_abun2","isolate2",
                                 "exp_abun1","exp_abun2")
results_comb_names <- lineages %>% select(Isolate,freschi2020) %>%
  inner_join(results_comb_names, by =c("Isolate" ="isolate2")) 
colnames(results_comb_names) <- c("isolate2","exp_lin1","isolate1",
                                  "exp_lin2", "read_cnt_1","read_cnt_2",
                                  "obs_lin1","obs_lin2","obs_abun1",
...
results_comb_names <- results_comb_names %>% 
  mutate(residual1 = abs(obs_abun1 - exp_abun1)) %>%
  mutate(residual2 = abs(obs_abun2 - exp_abun2))

results_comb_names <- results_comb_names %>%
  relocate(isolate1,exp_lin1,obs_lin1,
           exp_abun1,obs_abun1,read_cnt_1,residual1,
           isolate2,exp_lin2,obs_lin2,exp_abun2,obs_lin2,read_cnt_2,residual2)
results_comb_names %>% filter(exp_lin1 == obs_lin1, exp_lin2 == obs_lin2) 
results_comb_names %>% write_csv("mnt/tb_seqs/seq_simulation/amplicon_read_simulations/amplicon_alignment_wgsim/primer_v2/mixed_samples_diff_cov/demix_output_eval.csv")
